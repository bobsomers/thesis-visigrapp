...
\author{\authorname{Robert Edward Somers\sup{1} and Zo\"{e} J. Wood\sup{1} }
...
\abstract{As the quest for more realistic computer graphics marches steadily on, the
...
the use of the commodity hardware we already have at our disposal.Techniques such as normal mapping and level of detail have attempted to address
...
available system memory.We propose a distributed rendering architecture based on message-passing that
...
algorithms. At the heart of mimicking reality is describing the world we wish to show. In
...
volumes, algorithms that operate on them are inherently $n^3$.

Many methods have been developed for describing surfaces in computer
...
Meshes are efficient to process and easy for artists to work with because they represent discrete
...
Graphics and ray tracing in particular, has long been said to be a problem that is \emph{embarrassingly parallel},
...
amazing throughput of graphics primitives in real-time.While processor
...
scenes entirely in RAM.  Current strategies for parallelizing a renderer across a cluster of commodity
...
radiometrey, ray tracing\cite{appel:1968}, \cite{whitted:1980}, bounding volumes and Morton coding.
\paragraph{Radiometry and ray tracing}For a more  complete treatment of radiometry in the context of rendering algorithms and ray tracing in general, we refer
the reader to \emph{Physically Based Rendering} \cite{pbrt}.  The fundamentral parts of the 
radiometry model with respect to FlexRender are the following:
...
\paragraph{BVH} Bounding volume hierarchies, or BVHs, are essentially an application of binary
search to 3D space. BVHs are trees where each node is defined by a bounding volume, such as a box
...
in the tree.  After the tree structure is built using a typical linked-memory data structure,
...
The traversal algorithm of BVH trees is naturally recursive, but recursive implementations
...
explicitly exposes the state for capturing.  The iterative algorithm still requires a traversal stack of child nodes that
...
sent to each machine.  The Morton coding is relatively simple to implement. For example, say that
...
in the integer).  Computing the Morton code is simply a matter of calculating the 21-bit discretized
...
Large scale geometry is not new to computer graphics and a variety of algorithms have been developed
to simulate high resolution details.  For example, displacement maps\cite{krishnamurthy:1996} and 
normal maps using texture data\cite{cohen:1998} and \cite{cignoni:1998}.
...
or areas where the texture is scrunched or stretched.  Similarly, level of detail is a technique for replacing high resolution meshes with lower
resolution versions when they are some specified distance away from the viewer~\cite{clark:1976}.
This technique is also still heavily used in real-time graphics applications today. However it requires the
creation of multiple resolution meshes and algorithms to handle resolution changes.
...
a cache of rendering data in main memory. In 2011, \cite{tabellion:2011}
...
cache hit rates of over 99\%.  One year earlier, Pantaleoni et at. \cite{pantaleoni:2010} showed that their
...
\paragraph{General Purpose GPU Rendering}
\label{gpgpu}
...
architecture due to the problem's ``embarrassingly parallel'' nature. 
...
Finally, Northam and Smits \cite{northam:2011} presented a potential MapReduce\cite{dean:2004}
